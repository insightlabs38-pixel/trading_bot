...
def test_saved_predictions_reproduce_complete_cpu_leaderboard_without_training_import(
    ...
    fresh_report_path = tmp_path / "fresh-process-report"
    code = textwrap.dedent(
        ...
        dataset = read_prediction_artifact({str(artifact.path)!r})
    ...
    fresh_result = json.loads(completed.stdout)
    assert fresh_result["trial_id"] == "trial-a"
    assert fresh_result["mean_rank_ic"] > 0.9
    assert fresh_result["report_verified"] is True
